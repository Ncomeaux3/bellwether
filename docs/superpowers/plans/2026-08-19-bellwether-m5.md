...
**`src/ops/backup.ts`:** `backupSnapshot(db, dir, deps?): { path: string; bytes: number; pruned: string[] }` — `VACUUM INTO '<dir>/bellwether-<YYYYMMDD>.db'` (idempotent per day: if today's exists, replace via tmp+rename), prune local snapshots beyond 7 newest. `verifyBackup(livePath, snapshotPath)` opens both readonly and compares `snapshots`/`extractions`/`changes` counts. **Corrected during execution:** the original rule here was "within 2% OR 200 rows", which a review proved passes a completely EMPTY snapshot on the real archive (120 snapshots, 99 extractions — both under the flat 200-row floor), defeating the whole point. The rule is a proportional floor plus a small overage allowance: `snapshot >= floor(live * 0.97) && snapshot <= live + max(10, ceil(live * 0.02))`. (97%, not the 90% first drafted: at 10,000 rows a 90% floor tolerates 1,000 missing rows, so a 5%-truncated file still passes — the implementer caught that contradiction against this plan's own must-fail case and resolved it correctly.). It tolerates a stale snapshot (completeness is the property under test, not currency) and refuses a truncated one. CLI: `bw ops backup` (writes `runs` kind `backup`), `bw ops verify-backup [--file <path>]` (defaults to newest local snapshot; verifying the B2 copy happens host-side by restoring to a temp dir first — documented). Failures send a Telegram alert (injected).
**`ops/backup.sh`** (host, ~25 lines): sources `.env` for restic/B2 vars (exit 0 silently if unset); `restic backup data/backup` (the whole retained set, not just the newest — restic dedups, so the cost is negligible and a night whose push failed self-heals on the next run instead of leaving a permanent hole; ruled during execution) then `restic forget --keep-daily 7 --keep-weekly 4 --keep-monthly 12 --prune`. Crontab: nightly after publish; monthly verify line: restore newest to a tmpdir, run `docker compose exec -T bellwether pnpm bw ops verify-backup --file /data/restore-check.db` (restore into the bind mount), alert on failure via the command's own exit + heartbeat watchdog.
